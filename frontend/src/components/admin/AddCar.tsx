...
const AddCarForm: React.FC<AddCarFormProps> = ({ onCarAdded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(
        'http://localhost:3000/api/v1/cars/create',
        formData,
        {
          headers: {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="mb-3">
        ...
        <input type="text" className="form-control" id="car_name" value={car_name} onChange={(e) => setCarName(e.target.value)} required />
      </div>
      <div className="mb-3">
      ...
      <div className="mb-3">
        ...
        <input type="number" className="form-control" id="seat" value={seat} onChange={(e) => setSeat(Number(e.target.value))} required />
      </div>
      <div className="mb-3">
        <label htmlFor="transmission" className="form-label">Transmission</label>
        <input type="text" className="form-control" id="transmission" value={transmission} onChange={(e) => setTransmission(e.target.value)} required />
      </div>
      <div className="mb-3">
      ...
      <div className="mb-3 form-check">
        <input type="checkbox" className="form-check-input" id="availabillity" checked={availabillity} onChange={(e) => setAvailabillity(e.target.checked)} />
        <label className="form-check-label" htmlFor="availabillity">Available</label>
      </div>
      <div className="mb-3">
        <label htmlFor="start_rent" className="form-label">Start Rent</label>
        <input type="date" className="form-control" id="start_rent" value={start_rent} onChange={(e) => setStartRent(e.target.value)} required />
      </div>
      <div className="mb-3">
        <label htmlFor="end_rent" className="form-label">End Rent</label>
        <input type="date" className="form-control" id="end_rent" value={end_rent} onChange={(e) => setEndRent(e.target.value)} required />
      </div>
      <div className="mb-3">
        <label htmlFor="img" className="form-label">Image</label>
        <input type="file" className="form-control" id="img" onChange={(e) => setImg(e.target.files ? e.target.files[0] : null)} required />
      </div>
